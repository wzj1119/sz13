...
{
    memset(arg,0,256);
    sprintf(arg,"ffmpeg -i /home/wzj/worktest/15svedio/file/%d.mp4 -r 10 -b:v 320k -b:a 32k -y /home/wzj/worktest/15svedio/compressfileName/%d.mp4",fileId,fileId); //ffmppeg指令
      ...
      if(datalen==0) //当收到的数据中数据体长度为0时，把数据存成文件并存入MongoDB
      {
          // printf("end of file, 发送结束符号到终端设备!\n");
           ...
           if(2==eventType) //开关门图像
           {
                ...
                sprintf(callback->pName,"/home/wzj/worktest/15svedio/file/%d.jpg",Id);  //图片路径及名称
                ...
                sprintf(callback->Saveurl,"http://139.159.195.55:8080/home/wzj/worktest/15svedio/file/%d.jpg",Id);  //url路径及图像文件名称
            ...
            if(0==eventType || 1==eventType) //0：举报视频 1：紧急视频
            {
                ...
                memset(callback->pName,0,SavePSize);
                sprintf(callback->pName,"/home/wzj/worktest/15svedio/file/%d.mp4",Id);  //视频存储路径及名称
             // sprintf(vName,"/root/15sMedia/file/%d.mp4",Id);
                ...
                ffmnum=ffmpeg(Id,callback->Saveffmpeg);
                if(ffmnum!=0)
                {
                    qDebug("ffmpeg 压缩失败...");
                }
                ...
                sprintf(callback->Saveurl,"http://139.159.195.55:8080/home/wzj/worktest/15svedio/compressfileName/%d.mp4",Id); //url路径及视频文件名称
                MongoDB(callback->pName,callback->Savesim,callback->Saveeventtype,callback->Saveurl,callback->Savemediatype,callback->Savemediaformat); //将数据存入MongoDB
            }
